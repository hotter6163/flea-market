...
describe('ItemsServiceTest', () => {
  ...
  describe('findAll', () => {
    it('正常系', async () => {
      const expected = [];
      itemRepository.find.mockReturnValue(expected);
  ...
  describe('findOne', () => {
    it('正常系', async () => {
      ...
      itemRepository.findOne.mockReturnValue(expected);
      const result = await itemsService.findById('test-id');
      expect(result).toEqual(expected);
    });

    it('異常系: 商品が存在しない', async () => {
      itemRepository.findOne.mockReturnValue(null);
  ...
  describe('create', () => {
    it('正常系', async () => {
      ...
      itemRepository.createItem.mockReturnValue(expected);
  ...
  describe('updateStatus', () => {
    ...
    it('正常系', async () => {
      itemRepository.findOne.mockReturnValue(mockItem);
      await itemsService.updateStatus('test-id', mockUser2);
      expect(itemRepository.save).toHaveBeenCalled();
    });

    it('異常系: 自身の商品を購入しようとした', async () => {
      itemRepository.findOne.mockReturnValue(mockItem);
      await expect(
